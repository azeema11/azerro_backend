...
exports.recurrenceToMonthlyFactor = recurrenceToMonthlyFactor;
...
function detectFrequency(dates) {
    ...
    if (avgGap <= frequencyThresholds.DAILY)
        return "DAILY";
    if (avgGap <= frequencyThresholds.WEEKLY)
        return "WEEKLY";
    if (avgGap <= frequencyThresholds.MONTHLY)
        return "MONTHLY";
    if (avgGap <= frequencyThresholds.QUARTERLY)
        return "QUARTERLY";
    if (avgGap <= frequencyThresholds.HALF_YEARLY)
        return "HALF_YEARLY";
    if (avgGap <= frequencyThresholds.YEARLY)
        return "YEARLY";
    return null;
}
/**
...
function getPeriodDates(period, referenceDate = new Date()) {
    ...
    let end;
    switch (period) {
        case client_1.Periodicity.ONE_TIME:
...
function recurrenceToMonthlyFactor(recurrence) {
    switch (recurrence) {
